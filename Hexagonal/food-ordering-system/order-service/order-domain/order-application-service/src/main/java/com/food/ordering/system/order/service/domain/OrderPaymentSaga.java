...
public class OrderPaymentSaga implements SagaStep<PaymentResponse> {
    ...
    public void process(PaymentResponse paymentResponse) {
        ...
        //직접 메세지 발행이 아니라 그전에 DB 저장후 메세지 발행 == 아웃박스 패턴
    ...
    private SagaStatus[] getCurrentSagaStatus(PaymentStatus paymentStatus) {
        return switch (paymentStatus) {
            case COMPLETED -> new SagaStatus[] { SagaStatus.STARTED }; //결제 완료가 사가에서는 시작
            case CANCELLED -> new SagaStatus[] { SagaStatus.PROCESSING }; //결제 취소가  사가에서는 진행중
            case FAILED -> new SagaStatus[] { SagaStatus.STARTED, SagaStatus.PROCESSING }; // 결제 실패가 사가에서는 진행혹은 시작
        };
    }
    ...
    private OrderApprovalOutboxMessage getUpdatedApprovalOutboxMessage(String sagaId, OrderStatus orderStatus, SagaStatus sagaStatus) {
        Optional<OrderApprovalOutboxMessage> orderApprovalOutboxMessageResponse = approvalOutboxHelper.getApprovalOutboxMessageBySagaIdAndSagaStatus(UUID.fromString(sagaId), SagaStatus.COMPENSATING);
